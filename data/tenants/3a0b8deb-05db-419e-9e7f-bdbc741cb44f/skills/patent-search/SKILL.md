...
metadata:
  openclaw:
    ...
    envVars:
      ...
      - name: PATENT_URL_TEMPLATE
        required: false
        description: Patent web page template, e.g. https://www.9235.net/patent/{patent_id}.html
    install:
      - kind: uv
        package: requests
---
...
| | |
|---|---|
| **中文** | 对接 [9235](https://www.9235.net/api) 专利 API：检索、详情、权利要求、说明书、法律信息、引用/相似/同族专利、**企业画像**（工商全称）、统计与 Excel 导出；数据范围 `cn` / `all` / `us` / `jp` / `kr` / `tw` / `wo` / `ep`。 |
| **English** | [9235](https://www.9235.net/api) patent API client: search, detail, claims, description, legal events, citations, similar & **family** patents, **company portrait**, analytics, Excel export; scopes `cn`, `all`, `us`, `jp`, `kr`, `tw`, `wo`, `ep`. |

**凭证 Credentials**: `PATENT_API_TOKEN` 或 OpenClaw `skills.entries.patent-search.apiKey` · 申请 <https://www.9235.net/api/open>

**可选配置**（`config.json`）：`patentUrl`（如 `https://www.9235.net/patent` → 链到 `…/patent/CN115755970A.html`）、`show_patent_images`（详情内嵌摘要附图）。命令 `image` 可返回附图 Markdown 与可下载文件。

用户可说：「检索大疆无人机专利」「全球检索锂电池」「华为技术有限公司企业画像」「导出当前检索 Excel」「CN115755970A 的附图」。
...
- **法律信息**: 查看专利的法律事务信息
- **引用分析**: 查看专利的引用和被引用情况
- **相似专利**: 查找技术相近专利（`/api/patent/like`）
- **专利同族**: 同一专利族在不同国家/阶段的文献（`/api/patent/family`，命令 `family`）
- **企业画像**: 分析企业的专利布局和竞争力
- **文件下载**: 下载专利PDF全文和摘要附图
- **统计分析**: 专利维度统计分析

...
- **Cloud 专属**（`feature/channel-rental`）：每用户容器 Sidecar 策略、内存/CPU 覆盖等运营能力，**不在 Core `dev` 分支**
...
| 套餐 | Pro/Enterprise + 订阅有效 → 自动 container |
| 通道 | `workspace_mode`：local / container / null（自动）；Free 可在通道上强制 container 以启用 **自建 Skill** |
